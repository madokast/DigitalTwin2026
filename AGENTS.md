...
- **改 API = 更新 OpenAPI（+ fixtures）+ 双改代码 + 双跑测试**：`npm run openapi:lint`（Redocly）、`npm run test:openapi`、`cd faas && go test ./internal/contract/`，以及 `npm test` / `cd faas && go test ./...`。测试命令与门闸语义（单测 / 集成分开跑）见 README「测试」；Go 集成测自动加载仓库根 `.env.test`（同 Node `tests/setup.ts`，`go test -short` 为纯单元测）。CI：[`.github/workflows/ci.yml`](.github/workflows/ci.yml)（lint + 契约 + 无 DB 单元测；Node `tests/api` / Go httptest 集成在无安全 `DATABASE_URL`（host/库名含 `test` 或 `TestDigitalTwin`）时 Skip；unsafe 则拒绝。CI 检出不含 `.env.test`，Go 集成自动 Skip。可选 secrets `DATABASE_URL` 等启用 Node 集成测 job）。
