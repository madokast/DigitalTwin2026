...
- **改 API = 更新 OpenAPI（+ fixtures）+ 双改代码 + 双跑测试**：`npm run openapi:lint`（Redocly）、`npm run test:openapi`、`cd faas && go test ./internal/contract/`，以及 `npm test` / `cd faas && go test ./...`。CI：[`.github/workflows/ci.yml`](.github/workflows/ci.yml)（lint + 契约 + 无 DB 单元测；`tests/api` / Go httptest 集成在无 `DATABASE_URL` 时 Skip。可选 secrets `TEST_DATABASE_URL` 等启用 Node 集成测 job）。
- 网页 Settings 中的 **API Accelerate URL** 指向国内 FaaS Base URL（当前为 FC）；空则同源 Vercel。真实 URL **禁止进 git**。
- 生产密钥刷新：`npm run secrets:refresh-prod`（细节见 [`faas/providers/aliyun-fc/README.md`](faas/providers/aliyun-fc/README.md)）。
- **共享 Go 不得** import `faas/providers/*`。

# 部署原则（摘要）

- Vercel：海外主站；阿里云 FC：国内 API 加速（`faas/providers/aliyun-fc`）。
- API 保持标准 HTTP，便于移植。
