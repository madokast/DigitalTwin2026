...
- **改 API = 更新 OpenAPI（+ fixtures）+ 双改代码 + 双跑测试**：`npm run openapi:lint`（Redocly）、`npm run test:openapi`、`cd faas && go test ./internal/contract/`，以及 `npm test` / `cd faas && go test ./...`。CI：[`.github/workflows/ci.yml`](.github/workflows/ci.yml)（lint + 契约 + 无 DB 单元测；Node `tests/api` / Go httptest 集成在无安全 `DATABASE_URL`（host/库名含 `test` 或 `TestDigitalTwin`）时 Skip；unsafe 则拒绝。可选 secrets `DATABASE_URL` 等启用 Node 集成测 job）。
