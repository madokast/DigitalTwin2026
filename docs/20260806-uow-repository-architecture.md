...
6. **迁移 import / rename**（✅ 设计定案 2026-08-06，见 §10b 步骤 2）：先建 `FindByCriteria`（renameTag 复用，依赖顺序二次定案）→ `Exists` / `Update` + `RenameTag` 原语（insert 分支复用 `Save`；双端对称）；`ImportCounts` 移 `record` 包。
