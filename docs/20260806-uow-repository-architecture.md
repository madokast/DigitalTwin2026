...
| 8 | ~~`countTags`~~（取消） | — | — | **tagCounts 走 `FindByCriteria`（2026-08-06 二次定案）**：聚合函数改**数组版**（`AggregateTagCounts`/`aggregateTagCounts` 收 `[][]string`——FromDB 已 parse 并兜底脏数据，与跳过语义统一；`parseTagsJSONArray` 串版解析删除）；业务层分页循环收集 `Record.Tags` 数组 → 聚合（prefix 过滤留业务层）；`CountTags` 薄原语取消 |
...
6. **迁移 import / rename**（✅ 已完成）：先建 `FindByCriteria` → `Exists` / `Update` 原语 + `AcquireRenameLock`（insert 分支复用 `Save`；rename 业务层编排——`RenameAcrossRecords` 用 `FindByCriteria` + `Update` 分页循环；双端对称）；`ImportCounts` 移 `record` 包。**2026-08-07：rename 已被 normalize 替换**（`docs/20260805-tag-design.md` 拍板）——`RenameAcrossRecords` → `NormalizeAcrossRecords`（from 数组、多源一次变换 `normalizeTags`、每源分页扫）；`/api/admin/tags/normalize` 双端 + OpenAPI；前端 tags 页表单同步。
...
3. **UoW 步骤 8：迁移读路径**（✅ 已完成）——query/export/stats/summary/tags 的散落 SELECT 收进 `recordrepo`（`FindByCriteria` / `Count`，fake executor 单测查询条件），query.go 瘦身。**设计定案（2026-08-06 讨论）**：
   ...
   - **迁移映射盘点（2026-08-06 讨论记录）**：
     ...
     | `FetchTagCounts`（拉 tags + 聚合） | `FindByCriteria` 分页循环 + 数组版 `AggregateTagCounts`（`CountTags` 原语取消——2026-08-06 二次定案） |
...
4. **UoW 步骤 9：Service 化（最终横切，✅ 已完成）**——业务层自由函数 → Service struct/class 方法，构造注入 `db` + `uow`；**届时废除 httpx 可选函数字段 + nil 回落**（TransitionTodo/NotifyUser/FetchExportRecords → 接口注入）。依赖 2/3 完成后的完整原语集合。**设计定案（2026-08-06 讨论）**：
   - **粒度：按业务包各一个 Service**——`logapi.Service`（7 个 create/transition 方法）、`importapi.Service`（`ImportRecordsJSONL`）、`exportapi.Service`（`FetchExportRecords`）、`query.Service`（4 个 fetch）、`tags.Service`（`NormalizeAcrossRecords` + `AttachTag`/`DetachTag`）；包即边界，构造依赖最小。
