...
| 3 | `exists` / `insert` / `update` | `Exists(ctx, q, id) (bool, *MyError)` / `Insert(ctx, q, rec record.Record) *MyError` / `Update(ctx, q, rec record.Record) *MyError` | import 逐行 upsert 三原语（2026-08-06 定案：**保守复刻**，非 ON CONFLICT——否决理由见 §10b 步骤 2）；`Exists` 判存在 → 业务层分支 insert/update 并计数；**并发同 id 竞态：唯一索引拦截 → 500 整单回滚 = 正确失败语义，保留**；`Insert` 无 RETURNING（import 不消费行，与 `Save` 的 RETURNING 变体并存）；`Update` 全列覆盖（含 `::timestamptz` cast——与 `insert` 统一，行为等价；utc_offset 由 repo 内 `ParseHappenedAt(rec.HappenedAt)` 重解析，§4 两次解析成本原则）；`ImportCounts`（Inserted/Updated/Total）移 `record` 包（否则 recordrepo↔importapi 循环；Node 同步改 `ImportCounts`，双端词干一致） |
