...
- 用专用 API 创建待办；正文进 `raw_content`；代表状态用保留 tag `todo:{state}`。
...
- `happened_at` ← `created_at`；`raw_content` ← `content`；`numeric_value` = null。
...
**不**接受 `created_at`、`content`，也不接受改待办 `raw_content` / `objective_context` / 额外 tags 的字段——流转**只**改代表状态 tag，其它字段（含其它 tags）一律不动。**无**请求体 `suppress_notification`。

### 3.3 校验顺序与错误文案（英文，AI 可读；四类必须可区分）

建议严格按序短路，便于 AI 对症修改请求：

| 顺序 | 条件 | HTTP | `detail` 全文（定稿实现时双端字节一致） |
|------|------|------|----------------------------------------|
...
1. **UPDATE** 待办行：仅 tags 中的代表状态 tag 变为 `todo:{target}`；`raw_content` / `happened_at`（原行）/ `objective_context` / 其它 tags / 等字段不变。
2. **INSERT** 一条审计记录（见 §4）。
...
`{todo.happened_at}` 为流转**之前**读到的待办行经 `fromDB` 按 `utc_offset` 格式化后的带区串（ISO 8601），不得改写。

| `target` | `objective_context` |
|----------|---------------------|
| `completed` | `Complete a to-do {todo.id} created at {todo.happened_at}` |
| `cancelled` | `Cancel a to-do {todo.id} created at {todo.happened_at}` |
| `paused` | `Pause a to-do {todo.id} created at {todo.happened_at}` |
| `in_progress` | `Resume a to-do {todo.id} created at {todo.happened_at}` |

### 4.2 通知范围（**已拍板**）

一次 transition **成功**后：`notify_user` **恰好一次**（一律 schedule；真发与否仅看 `SUPPRESS_BOT_NOTIFICATION`）。

- 通知正文 = 一句话模板 `{Verb} a to-do {todo.id} created at {todo.happened_at}: {todo.raw_content}`，即**审计行 `objective_context` + `": "` + `raw_content`**（可由审计行两字段还原；非字节级一致）。
...
| 行判定 | JSON 行为 |
|--------|-----------|
| **待办行**（查询侧判定见 §1.3「略宽」规则） | 对外键名：`happened_at` → **`created_at`**，`raw_content` → **`content`**；其余字段默认序列化不变。响应中**不再出现**该行的 `happened_at` / `raw_content` 键（避免双键并存）。时间值仍按隐列 `utc_offset` 带区格式化（deform **只改键名**，不改回一律 `Z`；见 [`docs/20260803-utc-offset.md`](20260803-utc-offset.md) §6.1）。 |
| **审计行**（`todo:transition`）及其它非待办行 | **默认 toJSON**，仍为 `happened_at` / `raw_content`（与现网 Record 契约一致），**不做**别名。 |
...
- **Next**：在 query 组装响应处按行 `isTodoRecord(tags)`，映射为 `{ ...rest, created_at, content }`（去掉原 happened_at / raw_content 键），或小函数 `toQueryRecordJson(row)`。
...
2. **数据库真实列名仍是** `happened_at`、`raw_content`；变形只发生在 API 边界。
3. **流转**请求时间字段仍是 `happened_at`（审计时间），不要写成 `created_at`。
4. **审计行**若出现在 query 响应中仍是 `happened_at` / `raw_content`，没有 content 别名。
...
| 纠错 | 待办「完成/取消」走 transition + 审计，不靠改 `raw_content`；改错正文仍靠 Admin PATCH 或新待办（正文 PATCH 策略不在本篇扩大） |
...
- 测试：四类 transition 错误全文双端断言；创建 tags 组装；事务失败回滚；保留 tag 拒绝对通用 log；**query 待办行 JSON 键为 created_at/content、审计行仍为 happened_at/raw_content**。
...
14. **AI 手册预留（§5.4）**：必须向 AI 说明「对外别名 vs 库列 happened_at/raw_content」；本阶段不写手册。  
...
- **【已收】** transition notify：成功时**恰好一次**（一律 schedule；静音仅 env）；正文 = 审计行 `raw_content`（字节一致）；不另通知待办行。详见 §4.2、§8 第 8 条。  
- Admin PATCH 是否允许改待办正文（库列 `raw_content`；与保留 tag 的 PATCH 规则需对照现有 admin 行为）。  
...
| **范围外** | transition；**query** `records[]` 变形（期内 query 待办行仍可能是 `happened_at`/`raw_content`，已知短暂不一致）；审计行。 |
...
| **范围内** | OpenAPI：`LogTodoTransitionRequest`、成功体（`success`+`id`+`transition.{from,to}`）；`TransitionTodo` 事务；§3.3 四类英文错误 + `target` 非法文案；§4.1 审计 `raw_content` 模板；§4.2 notify 正文 = 审计文案；录入侧严「待办行」判定。 |
...
| **验证** | `?tag=todo:in_progress` 等：待办行无 `happened_at`/`raw_content` 键；`?tag=todo:transition` 仍为库契约键；Node/Go 宽松度一致。 |
