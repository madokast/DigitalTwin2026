...
- `POST /api/log/number`、`POST /api/log/text`、`POST /api/log/transaction`、`POST /api/log/body/weight`、`POST /api/log/todo`
...
- 触发：`POST /api/log/number|text|body/weight|todo` INSERT 成功后推单条；`POST /api/log/transaction` 整单成功后推一条 batch 摘要；均为 best-effort，通知失败不影响 `201`。
