...
- `POST /api/log/numbers`、`POST /api/log/text`、`POST /api/log/transactions`、`POST /api/log/body/weight`、`POST /api/log/todo`、`POST /api/log/todo/transition`
...
- `GET /api/query`、`/api/admin/records/stats`、`/api/query/tags`、`/api/query/transactions/summary`
- `GET /api/export/records`（ApiToken；`from?` + 必填 `limit`；NDJSON 有界缓冲下载，非 DB cursor 无限流）
- `POST /api/admin/tags/rename`（AdminToken）；`PATCH /api/admin/records/{id}` **已删除**（2026-08-04 彻底移除，效果 = 从未存在，见 [`docs/20260804-log-review.md`](../docs/20260804-log-review.md) §5）
...
- 触发：`POST /api/log/numbers|text|body/weight|todo` INSERT 成功后推单条；`POST /api/log/todo/transition` 成功后推**审计文案**一条；`POST /api/log/transactions` 整单成功后推一条 batch 摘要；`GET /api/export/records` / `POST /api/admin/import/records` 成功（含空）各推一次；均为 best-effort，通知失败不影响成功响应。
