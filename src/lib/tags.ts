...
/**
 * renameAcrossRecords 可注入的写库边界（与 Go `db.Querier` 假实现对称）。
 * 省略时走默认 Drizzle；单测传入 mock 断言 list / update 调用。
 */
...
const defaultRenameAcrossRecordsDb: RenameAcrossRecordsDb = {
  async listIdAndTags() {
    return db.select({ id: records.id, tags: records.tags }).from(records)
  },
  async updateTags(id, tagsJson) {
    await db.update(records).set({ tags: tagsJson }).where(eq(records.id, id))
  },
}

/**
 * 全表扫描 records，将 tags JSON 中 from 重命名为 to。
 * 与 Go `tags.RenameAcrossRecords` 对称的「笨」实现；性能优化须双端文档化后再破缺。
...
export async function renameAcrossRecords(
  from: string,
  to: string,
  store: RenameAcrossRecordsDb = defaultRenameAcrossRecordsDb,
): Promise<number> {
