...
describe('parseRecordQueryParams q OR grouping', () => {
  it('parenthesizes q OR when combined with tag (AND binds tighter than OR)', () => {
    ...
    expect(sql).toMatch(
      /like \$1 and \("records"\."value_text" like \$2 or "records"\."objective_context" like \$3 or "records"\."subjective_interpretation" like \$4 or "records"\."tags" like \$5\)/i,
    )
    expect(sql).not.toMatch(
      /like \$1 and "records"\."value_text" like \$2 or "records"\."objective_context"/i,
    )
  })
